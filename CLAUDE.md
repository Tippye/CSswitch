...
让 Claude Science 的模型推理走第三方 API（DeepSeek / 阿里通义千问 / 任意 Anthropic 或 OpenAI 兼容端点），保留 Science 那套 AI agent 科研体验，模型换成你自己的。类比 CC Switch 之于 Claude Code。
...
翻译代理 proxy/csswitch_proxy.py（默认 deepseek 原生透传 / qwen 翻译，可切）
   ...
   ▼
DeepSeek 原生 Anthropic 端点 / 阿里 DashScope（千问）/ 自定义 Anthropic/OpenAI 兼容端点
```
...
- **发布态 / 待办**：已公开于 github.com/Tippye/CSswitch；桌面 app 在 `desktop/`（Tauri **正常窗口**进程管家，已去托盘）。**当前 Latest 版本以 GitHub Releases / [`CHANGELOG.md`](CHANGELOG.md) 为准**；**待办队列与排期** → [`docs/known-issues.md`](docs/known-issues.md)。（此处不写具体版本号与「当前最优先」等易变状态，那些归 known-issues 与项目记忆 `memory/`。）
- **对外文案（脱敏）**：用户**可见**文案不露骨，别直说「越过 / 绕过登录」；主按钮用「一键开始」类中性说法。技术**内部**文档描述机制时可仍用「越过门票」。详见 `docs/known-issues.md` 第 1 条。
- **上游/模型**：默认 DeepSeek（原生 Anthropic 透传），可 `--provider qwen`（翻译），可 `--provider custom_anthropic`（透传，自定义端点），可 `--provider custom_openai`（翻译，自定义端点）。模型映射与选择器广告 id 见 `csswitch_proxy.py` 的 `PROVIDERS`。
- **自定义端点**：通过面板选择「自定义 Anthropic/OpenAI 兼容端点」，填写 API URL 和 Key。Rust 后端将 URL 通过 `CSSWITCH_CUSTOM_URL` 环境变量注入 proxy 子进程。
...
```
proxy/csswitch_proxy.py           【主】provider 可切代理（deepseek 默认 / qwen / custom_*）
...
- 切千问：`DASHSCOPE_API_KEY=... python3 proxy/csswitch_proxy.py --provider qwen --port 18991`
- 自定义 Anthropic：`CUSTOM_ANTHROPIC_API_KEY=... CSSWITCH_CUSTOM_URL=https://api.example.com/v1/messages python3 proxy/csswitch_proxy.py --provider custom_anthropic --port 18991`
- 自定义 OpenAI：`CUSTOM_OPENAI_API_KEY=... CSSWITCH_CUSTOM_URL=https://api.example.com/v1/chat/completions python3 proxy/csswitch_proxy.py --provider custom_openai --port 18991`
...
- 上游 key 在用户 shell 环境：`DEEPSEEK_API_KEY` / `DASHSCOPE_API_KEY` / `CUSTOM_ANTHROPIC_API_KEY` / `CUSTOM_OPENAI_API_KEY`（值不显示、不入库）。DashScope 兼容端点 `https://dashscope.aliyuncs.com/compatible-mode/v1`。
